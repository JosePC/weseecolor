...
├── canva-template.md        ← Canva field-name reference (predecessor workflow)
...
└── history/                 ← predecessor prompts kept for context
```
...
### `canva-template.md`
Reference for the *previous* card-production workflow, which used a Canva template with named fields populated via bulk upload / MCP. The plugin no longer goes through Canva — `renderer/` produces PDFs natively — but this document is preserved because the field names and the label/value conventions in it informed the renderer's JSON schema.
...
### `history/`
Predecessor prompts from the ChatGPT iteration of this work, kept for context on why the SKILL is structured the way it is. Not active reference material — don't update.
